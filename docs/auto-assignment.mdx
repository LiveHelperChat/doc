...
First, activate it by checking **Activate prioritized assignment** in the department's auto-assignment settings.

Optionally, you can also sort agents by **Assign chat by operator priority also** within the prioritized workflow. This applies the same `assign_priority` ordering used by the regular queue (see [Assign chat by operator priority](#assign-chat-by-operator-priority)).
...
| Setting | Description |
| --- | --- |
| **Activate prioritized assignment** | Master switch. When enabled, chats that fall within the priority range are processed by the prioritized assignment workflow first. |
...
| **Assign only priority operators** | When enabled, only operators who have **Participate only in department priority assignment queue** checked in their user settings are eligible for the priority queue. Operators without that flag stay in the regular queue. |
...
| **Minimum chat priority** | Chats with priority below this value are **not** processed by the prioritized workflow and fall through to the regular queue. |
| **Maximum chat priority** | Chats with priority above this value are **not** processed by the prioritized workflow and fall through to the regular queue. |
...
| **Min chat priority** (`chat_min_priority`) | Operator will only receive chats with priority `>=` this value from the prioritized workflow. `0` = no lower limit. |
| **Max chat priority** (`chat_max_priority`) | Operator will only receive chats with priority `<=` this value from the prioritized workflow. `0` = no upper limit. |
| **Participate only in department priority assignment queue** (`only_priority`) | When checked, the operator is **excluded** from the regular auto-assignment queue. They can only receive chats via the prioritized workflow. |
...
* **Participate only in department priority assignment queue** — operator-level setting (per department or department group).
* **Assign only priority operators** — department-level setting.
...
- ✅ **Activate prioritized assignment**  
- ❌ **Assign only priority operators** — keep disabled  
- Set **Minimum chat priority** to `6` and **Maximum chat priority** to `10`

**Per-operator configuration (for every operator):**

- ❌ **Participate only in department priority assignment queue** — keep disabled  
- Set **Min chat priority** and **Max chat priority** to `0` (or match the department range if you want to restrict some operators)

**Result:** Chats in `6`–`10` are processed by the prioritized workflow and can be assigned to any operator whose priority ranges match. Chats in `0`–`5` use the regular queue and are also available to all operators.
...
- ✅ **Activate prioritized assignment**  
- ✅ **Assign only priority operators** — enable this  
- Set **Minimum chat priority** to `6` and **Maximum chat priority** to `10`
...
- ✅ **Participate only in department priority assignment queue**  
...
- ✅ **Activate prioritized assignment**  
- ❌ **Assign only priority operators** — can be enabled or disabled depending on whether you want non-priority operators to also get priority chats (see Scenario 2 or 4)
...
**Result:** These operators are removed from the regular queue entirely. They only receive chats that pass through the prioritized workflow.
...
**Result:** Because **Assign only priority operators** is enabled at the department level, these operators (who do not have the operator-level flag) are excluded from the priority queue. They only receive regular chats.
...
| Setting | Value | Why |
| --- | --- | --- |
| **Activate prioritized assignment** | ✅ Enable | Turns on the priority workflow |
...
| **Assign only priority operators** | See sub-scenarios below | Controls whether normal operators are excluded from the priority queue |
...
| Operator | Min chat priority<br/>(`chat_min_priority`) | Max chat priority<br/>(`chat_max_priority`) | Participate only in priority queue<br/>(`only_priority`) | Handles |
| --- | --- | --- | --- | --- |
...
The `only_priority` flag (**Participate only in department priority assignment queue**) and the department's **Assign only priority operators** setting work together. Two common sub-scenarios:
...
**Department:** ❌ **Assign only priority operators** — disabled

**Per-operator `only_priority` flag:**

| Operator | Participate only in priority queue<br/>(`only_priority`) |
| --- | --- |
| Alice | ❌ disabled |
| Bob | ❌ disabled |
| Carol | ❌ disabled |
...
**Department:** ✅ **Assign only priority operators** — enabled
...
With **Assign only priority operators** enabled, Dave and Eve are excluded from the priority queue. Alice, Bob, and Carol must have `only_priority` enabled to be eligible. The pools are completely separated.

---

### Fallback behavior

When **Assign only priority operators** is enabled, the system tries the prioritized queue first. If no eligible priority operator is found (all are offline, at capacity, or priority ranges don't match), the chat falls through to the regular assignment workflow.
